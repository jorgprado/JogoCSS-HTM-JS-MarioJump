...
let isJumping = false; // Flag para controlar o pulo do Mario
let loop;
let startTime;
...
const jump = (event) => {
  const spaceBarKey = " ";
  if (event.key === spaceBarKey && !isJumping) { // Verifica se não está pulando
    isJumping = true; // Define que o Mario está pulando
    mario.classList.add('jump');
    setTimeout(() => {
      mario.classList.remove('jump');
      isJumping = false; // Define que o Mario terminou de pular
    }, 500);
